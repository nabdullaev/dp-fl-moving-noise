...
class LocalUpdateDP_Grad(object):
    def __init__(self, args, dataset=None, idxs=None):
        self.args = args
        self.loss_func = nn.CrossEntropyLoss()
        self.idxs_sample = np.random.choice(list(idxs), int(self.args.dp_sample * len(idxs)), replace=False)
    ...
    def train(self, net):
        ...
        for images, labels in self.ldr_train:
            ...
            if self.args.dp_mechanism != 'no_dp':
                ...
                self.clip_gradients(net)
                # 2) add gaussian noise to the average gradient
                self.noise_gradients(net)
    ...
    def noise_gradients(self, net):
        """
        Implement DP-SGD with Gaussian noise
        g = (1/L) * sum_i g_i
        g = g + N(0, sigma^2 * C^2 * I) / L
        """
        sigma = self.noise_scale
        L = len(self.idxs_sample)
        C = self.args.dp_clip
        ...
        for param in net.parameters():
            ...
            # draw noise from Gaussian distribution and scale it for the average gradient
            noise = torch.normal(mean=0, std=sigma * C, size=param.grad.shape, device=param.device)
            noise = noise / L
